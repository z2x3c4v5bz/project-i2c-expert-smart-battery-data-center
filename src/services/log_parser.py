...
from ..utils import canonical_hex, normalize_hex_token, safe_int, strip_us_unit
        ...
        if is_valid:
            if s_cnt == 1:
                ...
                if len(toks) < 3:
                    ...
                    device = toks[0].upper()
                    cmd = toks[1].upper()
                    bytes_le, is_nack = _bytes_from_tokens_low_to_high(toks, 2)
                    if len(bytes_le) < 1:
                        is_valid = False
                ...
                if '[S]' not in after_first:
                    ...
                    if len(toks1) != 2:
                        ...
                        cmd = toks1[1].upper()
                        bytes_le, is_nack = _bytes_from_tokens_low_to_high(toks2, 1)
                        if len(bytes_le) < 1:
                            is_valid = False
            else:
                is_valid = False
