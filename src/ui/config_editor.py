...
from ..models.sbs import ACCESS_TYPE, FUNCTION_TYPE, SbsCommandDef, SbsConfig
from ..services.config_io import save_config


class EditorState(enum.Enum):
    """Whether the editor has changes not yet written to disk."""
    CLEAN = 'clean'
    DIRTY = 'dirty'
...
class ConfigEditor(tk.Toplevel):
    """Editor for an in-memory ``SbsConfig``.

    State model (made explicit per the architecture proposal):

    * ``_snap_title`` / ``_snap_body`` — the last saved baseline. On a discarded
      close we restore ``cfg`` from this snapshot.
    * ``cfg`` itself — the working copy. "Apply Changes" commits the detail form
      into ``cfg.body``; Save writes ``cfg`` to disk and refreshes the snapshot.
    * ``_pending_bitfield`` — per-command bit-field edits that are staged (the
      "pending until Apply" label) and folded into ``cfg`` only on Apply Changes.
    * ``_state`` — CLEAN/DIRTY: are there changes not yet saved to disk?
    """
    ...
    def __init__(self, master: tk.Misc, cfg: SbsConfig, is_new: bool = False):
        ...
        self._state: EditorState = EditorState.CLEAN
        ...
        title_ent.bind('<KeyRelease>', lambda e: self._mark_dirty())
        ...
        self.fn_ent.bind('<KeyRelease>', lambda e: self._mark_dirty())
        ...
        self.acc_cb.bind('<<ComboboxSelected>>', lambda e: self._mark_dirty())
        ...
        unit_ent.bind('<KeyRelease>', lambda e: self._mark_dirty())
    ...
    def _mark_dirty(self):
        self._state = EditorState.DIRTY

    def _mark_clean(self):
        self._state = EditorState.CLEAN

    @property
    def _dirty(self) -> bool:
        return self._state is EditorState.DIRTY
